    ...
    <div>
        <div class="row">
            <div class="col-sm-7 text-right pt-2">
                <h2 class="h4 mb-4 text-info">لیست کاربران</h2>
            </div>
            <div class="col-sm-5 text-left">
                <router-link :to="{name: 'users.create'}" class="btn btn-info m-1 item ">
        ...
        <div v-if="this.records.total > 0">
            <table class="lian-table table table-hover table-borderless border-0">
                ...
                <tr>
                    ...
                    <th class="md" scope="col">نام کاربری</th>
                    <th class="md" scope="col">وضعیت</th>
                    <th class="md" scope="col">شماره تماس</th>
                    <th class="md" scope="col">تعداد نوشته</th>
                    <th class="md" scope="col">تعداد محصول</th>
                ...
                <tr v-for="(record, index) in records.data">
                    ...
                    <td>
                        <a :href="'https://www.lianonline.ir/user/'+record.id" target="_blank"><span>{{record.username?record.username:'----'}}</span></a>
                    </td>
                    <td>{{record.status === 'active' ? 'فعال' : 'غیرفعال'}}</td>
                    <td>{{record.mobile}}</td>
                    <td>{{record.post_count}}</td>
                    <td>{{record.product_count}}</td>
                    <td>{{record.wallet.amount}}</td>
                    <td class="text-center">
                        <router-link :to="{name: 'users.edit', params: {'id': record.id}}" class="btn btn-sm btn-success">
                            <span>ویرایش</span>
                        </router-link>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                'records': new Records(),
                'base_url': '/management/api/user/list',
                'filters': {},
                'is_filtering': false,
                'errors': new Errors(),
                'message': ''
        ...
        methods : {
            ...
            remove(id) {
                ...
                if (answer) {
                    window.axios.post('/management/api/user/delete/' + id, {})
                        .then(response => {
                            if (response.data.returnCode === 0)
                            {
