<template>
    <div @keydown="errors.clear($event.target.name)">
        ...
        <div class="row">
            <div class="col-12">
                <hr>
                <button v-if="fields.id > 0" class="btn btn-success rounded-pill py-2 px-5 font-14" @click="saveCategory()">ذخیره</button>
                <router-link :to="{name: 'category.list'}" class="btn btn-secondary rounded-pill py-2 px-5 font-14">
                    <span>بازگشت</span>
                </router-link>
            </div>
        </div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                'errors': new Errors(),
                'message': '',
                'base_url': '/management/api/category',
                'mainCategories': []
            };
        },
        methods : {
            getMainCategories() {
                window.axios.post(this.base_url + '/main-categories', {})
                    .then(response => {
