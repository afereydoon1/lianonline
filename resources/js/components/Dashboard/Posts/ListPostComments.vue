    ...
    <div>
        <div class="row">
            <div class="col-sm-7 text-right pt-2">
                <h2 class="h4 mb-4 text-info">لیست دیدگاه های نوشته ها</h2>
            </div>
        </div>
        ...
        <div v-if="this.records.total > 0">
            <table class="lian-table table table-hover table-borderless border-0">
                ...
                <tr v-for="(record, index) in records.data" :key="record.id">
                    ...
                    <td>
                        <router-link v-if="record.commentable" :to="{name: 'posts.edit', params: {'id': record.commentable.id}}">
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                'records': new Records(),
                'base_url': '/management/api/post/comments/list',
                'filters': {},
                'is_filtering': false,
                'errors': new Errors(),
                'message': ''
        ...
        methods : {
            ...
            remove(id) {
                ...
                if (answer) {
                    window.axios.post('/management/api/post/comments/delete/' + id, {})
                        .then(response => {
                            if (response.data.returnCode === 0)
                            {
