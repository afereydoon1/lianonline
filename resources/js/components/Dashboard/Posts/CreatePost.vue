<template>
    <div @keydown="errors.clear($event.target.name)">
        <h2 class="h4 mb-4 text-info">افزودن نوشته ی جدید</h2>
        ...
        <div class="row">
            ...
            <div class="col-md-4">
                <div class="form-group mb-4" :class="{'has-error' : errors.has('creator')}">
                    ...
                    <select id="user" name="user" v-model="fields.creator" class="form-control">
                        <option v-for="user in users" :key="user.id+user.type" :value="user">{{user.name?user.name:user.mobile}}</option>
                    </select>
                    <span class="error-text" v-if="errors.has('creator')">{{ errors.get('creator')}}</span>
                </div>
            </div>
        </div>
            ...
            filesUpdated(images) {
                ...
                if(images.length > 0) {
                    if(images[images.length-1].file_id > 0) {
                        this.fields.image_id = images[images.length-1].file_id;
                    }
                }
            },
            getUsers() {
