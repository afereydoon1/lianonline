    ...
    <div>
        <div class="row">
            <div class="col-sm-7 text-right pt-2">
                <h2 class="h4 mb-4 text-info">لیست برچسب ها</h2>
            </div>
            <div class="col-sm-5 text-left">
                <router-link :to="{name: 'tags.create'}" class="btn btn-info m-1 item ">
        ...
        <div v-if="this.records.total > 0">
            <table class="lian-table table table-hover table-borderless border-0">
                ...
                <tr>
                    ...
                    <th class="md" scope="col">عنوان برچسب</th>
                    <th class="md" scope="col">تعداد پستها</th>
                    <th class="md" scope="col">تعداد محصولات</th>
                ...
                <tr v-for="(record, index) in records.data" :key="record.id">
                    ...
                    <td class="text-center">
                        <router-link :to="{name: 'tags.edit', params: {'id': record.id}}" class="btn btn-sm btn-success">
                            <span>ویرایش</span>
                        </router-link>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                'records': new Records(),
                'base_url': '/management/api/tag/list',
                'filters': {},
                'is_filtering': false,
                'errors': new Errors(),
                'message': ''
        ...
        methods : {
            ...
            remove(id) {
                ...
                if (answer) {
                    window.axios.post('/management/api/tag/delete/' + id, {})
                        .then(response => {
                            if (response.data.returnCode === 0)
                            {
