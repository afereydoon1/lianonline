<template>
    <div @keydown="errors.clear($event.target.name)">
        <h2 class="h4 mb-4 text-info">ویرایش محصول : {{fields.title}}</h2>
        ...
        <div class="row">
            <div class="col-md-3">
                <div class="form-group mb-4" :class="{'has-error' : errors.has('marketer_percent')}">
                    ...
                    <input type="number" v-model="fields.marketer_percent" id="marketer_percent" name="marketer_percent" class="form-control ltr" placeholder="" min="0" max="60">
            ...
            <div class="col-md-3">
                <div class="form-group mb-4" :class="{'has-error' : errors.has('has_discount')}">
                    ...
                    <select v-model="new_has_discount" id="has_discount" name="has_discount" class="form-control">
            ...
            <div class="col-md-3" v-if="new_has_discount == 1">
        ...
        <div class="row mt-5">
            <div class="col-12">
                ...
                :images="fields.images?fields.images:[]"
...
<script>
    ...
    export default {
        ...
        watch: {
            ...
            new_has_discount: function(newVal) {
               this.fields.has_discount = newVal;
               if(this.fields.has_discount == 0) {
                   this.fields.discounted_price = 0;
               }
                //this.$refs.tags.items = this.tags;
            },
        },
        methods : {
                ...
                window.axios.post(this.base_url + '/show/' + this.$route.params.id, {})
                    .then(response => {
                        if (response.data.returnCode === 0)
                        {
                            ...
                            if (this.fields.discounted_price != undefined && this.fields.discounted_price != null && this.fields.discounted_price > 0) {
                                this.fields.has_discount = 1;
                                this.new_has_discount = 1;
                            }else{
                                this.fields.discounted_price = 0;
                                this.fields.has_discount = 0;
                                this.new_has_discount = 0;
                            }
                            let categories = [];
                            for (let i = 0; i < response.data.data.record.categories.length; i++) {
                                categories.push(response.data.data.record.categories[i].id);
                            }
                            this.fields.categories = categories;

                            if (this.fields.file && typeof this.fields.file.id !== typeof undefined) {
                                this.file = this.fields.file;
                                this.fields.file = this.fields.file.id;
                            }
            ...
            saveProduct() {
                ...
                for (let i = 0; i < this.fields.images.length; i++) {
                    if(this.fields.images[i].download_path != undefined) {
                        data['images'].push(this.fields.images[i].id);
                    }else{
                        if(this.fields.images[i].success) {
                            data['images'].push(this.fields.images[i].file_id);
                        }
                    }
                }
