    ...
    <div>
        <div class="row">
            <div class="col-sm-7 text-right pt-2">
                <h2 class="h4 mb-4 text-info">لیست سفارشات ناموفق</h2>
            </div>
        </div>
        ...
        <div v-if="this.records.total > 0">
            <table class="lian-table table table-hover table-borderless border-0">
                ...
                <tr>
                    ...
                    <th class="md" scope="col">نام کاربر</th>
                    <th class="md" scope="col">شماره تماس</th>
                    <th class="md" scope="col">مبلغ کل</th>
                    <th class="md" scope="col">وضعیت</th>
                    <th class="md" scope="col">تاریخ</th>
                ...
                <tr v-for="(record, index) in records.data" :key="record.id">
                    ...
                    <td class="text-center">
                        <router-link :to="{name: 'orders.show', params: {'id': record.id}}" class="btn btn-sm btn-success">
                            <span>مشاهده</span>
                        </router-link>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                'records': new Records(),
                'base_url': '/management/api/orders/list',
                'filters': {},
                'is_filtering': false,
                'errors': new Errors(),
                'message': ''
        ...
        methods : {
            ...
            remove(id) {
                ...
                if (answer) {
                    window.axios.post('/management/api/orders/delete/' + id, {})
                        .then(response => {
                            if (response.data.returnCode === 0)
                            {
        ...
        mounted() {
            this.getList(this.base_url + '/failed');
        },
        filters: {
            jalaliDate(date) {
